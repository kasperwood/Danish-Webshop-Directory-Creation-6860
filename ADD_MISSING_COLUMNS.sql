-- Add all missing columns to webshops table
...
WHERE 
  mobilepay_accepted IS NULL OR 
  headline_bg_color IS NULL OR 
  headline_speed IS NULL OR 
  usp_items IS NULL OR 
  categories IS NULL;
...
SELECT id, name, mobilepay_accepted, headline_bg_color, headline_speed, 
       array_length(usp_items, 1) as usp_count,
